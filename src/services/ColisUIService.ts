...
declare global {
    interface Window {
        ...
        voirColis: (id: string) => void;  // Ajout de la déclaration pour voirColis
    }
}
...
export class ColisUIService {
    ...
    private renderColis(colis: Colis[]) {
    ...
    private setupForm() {
        const form = document.getElementById('new-colis-form') as HTMLFormElement;
        if (!form) return;
    ...
    private setupSearch(): void {
        ...
        this.searchInput.addEventListener('input', async (e) => {
            ...
            const filteredColis = allColis.filter(colis => 
                colis.getCodeSuivi().toLowerCase().includes(searchTerm) ||
    ...
    private getEtatClass(etat: string): string {
        ...
        return classes[etat as keyof typeof classes] || 'bg-gray-100 text-gray-800';
    }
}
