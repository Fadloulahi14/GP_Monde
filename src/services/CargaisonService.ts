...
export class CargaisonService {
    ...
    async getAll(page: number = 1, limit: number = 7): Promise<{cargaisons: Cargaison[], total: number}> {
        const response = await fetch(this.apiUrl);
        if (!response.ok) throw new Error(`Erreur HTTP : ${response.status}`);
        const cargaisonsData = await response.json();
        
        cargaisonsData.sort((a: any, b: any) => {
        ...
        const cargaisons = paginatedData.map((data: any) => {
            const colisCargaison = colis.filter((c: any) => c.cargaisonId === data.id);
            const poidsTotal = colisCargaison.reduce((total: number, c: any) => total + (c.Poids || 0), 0);
            const prixTotal = colisCargaison.reduce((total: number, c: any) => {
                const prixCalcule = (c.Poids || 0) * 500;
                return total + Math.max(prixCalcule, 10000);
            }, 0);

            return Cargaison.fromObject({
                ...data,
                poids_total: poidsTotal,
                prix_total: prixTotal,
                colis: colisCargaison
            });
        });
    ...
    async create(formData: FormData): Promise<any> {
        ...
        const poidsMax = parseFloat(formData.get('poidsMax') as string) || 0;
        const lieuDepart = formData.get('lieuDepart') as string;
        const lieuArrivee = formData.get('lieuArrivee') as string;
    ...
    async getColisDisponibles(): Promise<any[]> {
        ...
        return allColis.filter((colis: any) => !colis.cargaisonId);
    }

    async fermerCargaison(cargaisonId: string): Promise<void> {
    ...
    async rouvrirCargaison(cargaisonId: string): Promise<void> {
    ...
    async updateStatus(cargaisonId: string, newStatus: string): Promise<void> {
        try {
            ...
            if (!cargaisonResponse.ok) throw new Error(`HTTP error! status: ${cargaisonResponse.status}`);

            const colisResponse = await fetch(`${ApiService.getUrl('colis')}?cargaisonId=${cargaisonId}`);
            const colis = await colisResponse.json();

            const updatePromises = colis.map(async (colis: any) => {
                let colisStatus = '';
                switch (newStatus) {
                    case 'en_cours':
