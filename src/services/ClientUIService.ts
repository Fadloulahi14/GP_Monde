...
// Déclaration globale avec le bon type
declare global {
    interface Window {
...
export class ClientUIService {
    ...
    async loadClients(containerId: string) {
        try {
            ...
            const expediteurs = colis
                ...
                .reduce((acc, col) => {
                    ...
                    return acc;
                }, {} as Record<string, ExpediteurData>);
            ...
            container.innerHTML = expediteursData
                .map((exp: ExpediteurData) => this.generateClientRow(exp))
                .join('');
    ...
    private attachViewDetailsEvents(expediteursData: ExpediteurData[]) {
        const voirClient: typeof window.voirClient = (clientId) => {
        ...
        const closeClientModal: typeof window.closeClientModal = () => {
    ...
    private generateExpediteurDetails(expediteur: ExpediteurData): string {
        return `
            <div class="p-6">
    ...
    private generateColisList(colis: Colis[]): string {
        if (!colis.length) return '<p class="text-gray-500">Aucun colis</p>';
    ...
    private generateClientRow(expediteur: ExpediteurData): string {
    ...
    public attachSearch(searchInputId: string, containerId: string) {
        const searchInput = document.getElementById(searchInputId) as HTMLInputElement;
        if (!searchInput) return;

        searchInput.addEventListener('input', (e) => {
            const searchTerm = (e.target as HTMLInputElement).value.toLowerCase();
            const rows = document.querySelectorAll(`#${containerId} tr`) as NodeListOf<HTMLTableRowElement>;
