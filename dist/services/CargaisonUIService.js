...
export class CargaisonUIService {
    ...
    async calculateRoute(depart, arrivee) {
        try {
            if (this.marker1)
            ...
            const distance = this.calculateDistance(departCoords.lat, departCoords.lon, arriveeCoords.lat, arriveeCoords.lon);
            return {
    ...
    async showCargaisonDetails(cargaisonId) {
        var _a;
        try {
            ...
            if (!colisResponse.ok)
                throw new Error(`HTTP error! status: ${colisResponse.status}`);
                                    ...
                                    <div class="bg-blue-50 p-4 rounded-lg">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="mt-1 text-lg font-semibold">${((_a = cargaison.trajet) === null || _a === void 0 ? void 0 : _a.distance) || 0} km</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    attachModalEvents(modal, modalId, cargaisonId) {
        ...
        const form = document.getElementById(`cargaison-status-form-${cargaisonId}`);
        if (form) {
            form.addEventListener('submit', async (e) => {
                e.preventDefault();
                const statusSelect = form.querySelector('#cargaison-status');
                if (statusSelect) {
                    try {
